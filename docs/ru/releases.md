...
Обычный коммит, push ветки и pull request не запускают проверки, сборку установщиков или публикацию. Единственный автоматический запуск — отправка нового тега вида vX.Y.Z. Номер тега должен совпадать с версией приложения; это проверяется до запуска матрицы. Quality вызывается только из Release. Обновление или удаление существующего тега не выпускает релиз. Создавать GitHub Release вручную не нужно. Уже запущенные старые workflow можно отменить отдельно в Actions.
